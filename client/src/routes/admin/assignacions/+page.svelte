<script lang="ts">
  ...
  async function carregar() {
    error = null;
    try {
      const r = await fetch('http://localhost:8080/tutors/assignacions', { headers: authHeaders() });
  ...
  async function crear() {
    error = null;
    try {
      const r = await fetch('http://localhost:8080/tutors/assignacions', {
  ...
  async function importar() {
    ...
    try {
      ...
      const r = await fetch('http://localhost:8080/tutors/import', {
        method: 'POST', headers: { ...authHeaders(), 'Content-Type': 'application/json' },
