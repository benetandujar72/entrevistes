<script lang="ts">
  import { onMount } from 'svelte';
  import { authStore } from '$lib/stores/auth';
  import Button from '$lib/components/ui/Button.svelte';
  import Card from '$lib/components/ui/Card.svelte';
  
  let isLoggedIn = false;
  
  $: authStore.subscribe(store => {
    isLoggedIn = store.isAuthenticated;
  });
  
  function loginWithGoogle() {
    if (window.google) {
      window.google.accounts.id.prompt();
    }
  }
</script>

<svelte:head>
  <title>Inici - Sistema d'Entrevistes</title>
</svelte:head>

<div class="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-50 to-primary-100 dark:from-gray-900 dark:to-gray-800">
  <div class="max-w-md w-full space-y-8">
    <div class="text-center">
      <div class="mx-auto h-16 w-16 bg-primary-600 rounded-full flex items-center justify-center mb-4">
        <span class="text-white font-bold text-2xl">E</span>
      </div>
      <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
        Sistema d'Entrevistes
      </h1>
      <p class="mt-2 text-gray-600 dark:text-gray-400">
        INS Bitacola - Gestió d'entrevistes i alumnes
      </p>
    </div>
    
    <Card>
      {#if isLoggedIn}
        <div class="text-center space-y-4">
          <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
            Benvingut al sistema
          </h2>
          <p class="text-gray-600 dark:text-gray-400">
            Ja estàs autenticat. Pots accedir al dashboard per començar.
          </p>
          <Button href="/dashboard" variant="primary" size="lg">
            Anar al Dashboard
          </Button>
        </div>
      {:else}
        <div class="text-center space-y-4">
          <h2 class="text-xl font-semibold text-gray-900 dark:text-white">
            Iniciar sessió
          </h2>
          <p class="text-gray-600 dark:text-gray-400">
            Accedeix amb el teu compte de Google per continuar
          </p>
          <Button on:click={loginWithGoogle} variant="primary" size="lg">
            <svg class="w-5 h-5 mr-2" viewBox="0 0 24 24">
              <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Iniciar sessió amb Google
          </Button>
        </div>
      {/if}
    </Card>
    
    <div class="text-center text-sm text-gray-500 dark:text-gray-400">
      <p>Només usuaris amb compte @insbitacola.cat</p>
    </div>
  </div>
</div>

<script>
  declare global {
    interface Window {
      google: any;
    }
  }
</script>
